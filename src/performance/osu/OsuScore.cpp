...
void OsuScore::computeAimValue(const Beatmap &beatmap)
{
	...
	f32 estimateSliderEndsDropped = std::min(estimateDifficultSliders, std::min(static_cast<f32>(_num100 + _num50 + _numMiss), maxCombo - _maxCombo));
	...
	_aimValue *= std::max(sliderFactor, sliderNerfFactor);
